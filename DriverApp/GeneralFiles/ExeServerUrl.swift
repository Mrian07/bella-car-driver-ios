...
class ExeServerUrl: NSObject, URLSessionDelegate, URLSessionTaskDelegate, URLSessionDataDelegate {
    ...
    */
    func executePostProcess(completionHandler: @escaping CompletionHandler) {
        ...
        /* FOR POOL.*/
        for (_, value) in dict_data! {
            if (value == "GenerateTrip" || value == "DriverArrived" || value == "ProcessEndTrip" || value == "CollectPayment" || value == "submitRating"){
                GeneralFunctions.saveValue(key: "STOP_POOL_REQUEST", value: true as AnyObject)
                break
            }
        }
        /* FOR POOL.*/
    ...
    private func continuePostProcess(postString:String, request: NSMutableURLRequest, completionHandler: @escaping CompletionHandler){
        ...
        */
        ...
        if(UserDefaults.standard.object(forKey: Utils.USER_PROFILE_DICT_KEY) != nil){
            onlyDeliverAll = (GeneralFunctions.getValue(key: Utils.USER_PROFILE_DICT_KEY) as! String).getJsonDataDict().getObj(Utils.message_str).get("ONLYDELIVERALL")
            deliverAll = (GeneralFunctions.getValue(key: Utils.USER_PROFILE_DICT_KEY) as! String).getJsonDataDict().getObj(Utils.message_str).get("DELIVERALL")
        }
        
        var postString = "\(postString)&tSessionId=\(GeneralFunctions.getSessionId())&GeneralMemberId=\(GeneralFunctions.getMemberd())&GeneralUserType=\(Utils.appUserType)&GeneralDeviceType=\(Utils.deviceType)&GeneralAppVersion=\(Utils.applicationVersion())&vTimeZone=\(DateFormatter().timeZone.identifier)&vUserDeviceCountry=\(Utils.getDeviceCountryCode())&deviceHeight=\(((Application.screenSize.height - GeneralFunctions.getSafeAreaInsets().top) * UIScreen.main.scale))&deviceWidth=\(Application.screenSize.width * UIScreen.main.scale)&APP_TYPE=\(CUSTOM_APP_TYPE)"
        
    
        if(onlyDeliverAll == "Yes"){
            postString = "\(postString)&eSystem=DeliverAll"
        }else if(deliverAll == "No"){
            postString = "\(postString)&eSystem="
        }
        ...
        Utils.printLog(msgData: "requestURL:\(postString)")
        
        let task = URLSession.shared.dataTask(with: request as URLRequest, completionHandler: {data, response, error -> Void in
            
            /* FOR POOL.*/
            GeneralFunctions.saveValue(key: "STOP_POOL_REQUEST", value: false as AnyObject)
            /* FOR POOL.*/
...
//        let imageData = UIImageJPEGRepresentation(image, 0.8)
        ...
        */
        ...
        dict_data?["deviceHeight"] = "\((Application.screenSize.height - GeneralFunctions.getSafeAreaInsets().top) * UIScreen.main.scale)"
        dict_data?["deviceWidth"] = "\(Application.screenSize.width * UIScreen.main.scale)"
        dict_data?["APP_TYPE"] = "\(CUSTOM_APP_TYPE)"
        
        if(onlyDeliverAll == "Yes"){
            dict_data?["eSystem"] = "DeliverAll"
        }else if(deliverAll == "No"){
            dict_data?["eSystem"] = ""
        }
