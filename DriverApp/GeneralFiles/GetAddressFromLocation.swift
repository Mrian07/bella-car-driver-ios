...
class GetAddressFromLocation: NSObject {
    ...
    func executeProcess(isOpenLoader:Bool, isAlertShow:Bool){
        ...
        let geoCodeUrl = "https://maps.googleapis.com/maps/api/geocode/json?latlng=\(location!.coordinate.latitude),\(location!.coordinate.longitude)&key=\(Configurations.getGoogleServerKey())&language=\(Configurations.getGoogleMapLngCode())&sensor=true"
