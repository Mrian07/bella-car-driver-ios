...
open class BTNavigationDropdownMenu: UIView {
    ...
    public init(navigationController: UINavigationController? = nil, containerView: UIView = UIApplication.shared.keyWindow!, title: String, items: [NSDictionary]) {
        ...
        var titleWidth = self.configuration.arrowPadding + self.configuration.arrowImage.size.width
        titleWidth = titleWidth * 2
        // Set frame
        let frame = CGRect(x: 0, y: 0, width: titleSize.width + titleWidth, height: self.navigationController!.navigationBar.frame.height)
...
//            }
    ...
    func showMenu() {
        ...
        let yOrigin = CGFloat(self.items.count) * self.configuration.cellHeight
        let yOriginOffset = 1 - yOrigin - 300
        
        // Animation
        self.tableView.frame.origin.y = yOriginOffset
    ...
    @objc func hideMenu() {
        ...
        UIView.animate(withDuration: self.configuration.animationDuration, delay: 0, options: UIView.AnimationOptions(), animations: {
            self.tableView.frame.origin.y = yOriginOffset
            self.backgroundView.alpha = 0
        }, completion: { (isAnimCompleted) in
            if(self.isShown == false && self.tableView.frame.origin.y == yOriginOffset) {
                self.menuWrapper.isHidden = true
            }
        })
