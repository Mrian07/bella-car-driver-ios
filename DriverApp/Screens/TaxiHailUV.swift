...
//        }
        ...
        if(requestPickUpView != nil && ((self.contentView.subviews.count + self.view.subviews.count) > self.currentTotalViews)){
            return
        }
    ...
    func openRequestPickUpView(){
        ...
        currentTotalViews = self.contentView.subviews.count + self.view.subviews.count
...
//        self.bookNowBtn.setButtonTitleColor(color: UIColor(hex: 0x6b6b6b))
    ...
    func getDirectionData(){
        ...
        if(bookNowBtn != nil){
         ...
         let directionURL = "https://maps.googleapis.com/maps/api/directions/json?origin=\(self.pickUpLocation!.coordinate.latitude),\(self.pickUpLocation!.coordinate.longitude)&destination=\(destLocation!.coordinate.latitude),\(destLocation!.coordinate.longitude)&key=\(Configurations.getGoogleServerKey())&language=\(Configurations.getGoogleMapLngCode())&sensor=true"
...
//        bgViewTapGue.addTarget(self, action: #selector(self.cancelSurgeView))
        ...
        self.surgePayAmtLbl.text = (selectedVehicleTypeItem.get("SubTotal") == "" || rentalInfoLbl.isHidden == false) ? "\(self.generalFunc.getLanguageLabel(origValue: "", key: "LBL_PAYABLE_AMOUNT"))" : "\(self.generalFunc.getLanguageLabel(origValue: "Approx payable amount", key: "LBL_APPROX_PAY_AMOUNT")): \(Configurations.convertNumToAppLocal(numStr: selectedVehicleTypeItem.get("SubTotal")))"
